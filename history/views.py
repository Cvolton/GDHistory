...
def index(request):
	...
	recently_added = Level.objects.order_by('-pk').filter(cache_search_available=False)[:5]
...
def view_level(request, online_id=None, record_id=None):
	...
	level_records = all_levels.filter(level__online_id=online_id).exclude(level_version=None).prefetch_related('level').prefetch_related('level_string').prefetch_related('real_user_record__user').annotate(oldest_created=Min('save_file__created'), real_date=Coalesce('oldest_created', 'server_response__created')).order_by('-real_date')
...
def daily(request):
	...
	for record in levels:
		...
		if record.cache_daily_id < 100000:
			...
			elif record.cache_daily_id < 1492: records[2020].append(record) #estimated - unable to determine if off by 20 or 21 from said list, assuming 20
