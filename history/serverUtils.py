...
def create_level_record_from_data(level_data, level_object, record_type, server_response, *args, **kwargs):
	...
	try: #TODO: merge the 2 cases
		return LevelRecord.objects.get(level=level_object,
			...
			timestamp = assign_key_no_pop(level_data, 57),
			song_ids = assign_key_no_pop(level_data, 52),
			sfx_ids = assign_key_no_pop(level_data, 53),
		...
		record = LevelRecord(level=level_object,
			...
			timestamp = assign_key(level_data, 57),
			song_ids = assign_key(level_data, 52),
			sfx_ids = assign_key(level_data, 53),
			record_type = record_type,
...
def process_get(response_json):
	...
	for item in song_array:
		...
		record = create_song_record_from_data(item, song_object, SongRecord.RecordType.LEVEL_INFO, response_object.created, decode_link=True)
